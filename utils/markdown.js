// Credit: github.com/sarvasvkulpati/markdown
...
class Parser {
  ...
  parseList() {
    ...
    while (
      this.tokenLineReader.hasNext() &&
      (this.tokenLineReader.peek()[0] == "LIST_ITEM" ||
        this.tokenLineReader.peek()[1] == "LIST_ITEM")
    ) {
      this.tokenReader = new Reader(this.tokenLineReader.next());
...
function parseDomNodes(tree) {
  ...
  let attributes = "";
  if (tree.attributes) {
    attributes = Object.keys(tree.attributes)
      .map((key) => {
